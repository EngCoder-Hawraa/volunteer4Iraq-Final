...
from .forms import CreateNewIntity, CreateNewUser
...
def register_intities(request):
    # if request.user.is_authenticated:
        # //this code save the user. So, enter directly without anything(information)
        # return redirect('/admin_home')
    if request.method=="POST":
        form = CreateNewIntity(request.POST)
        if form.is_valid():
            email = form.cleaned_data['email']
            password = form.cleaned_data['password1']
            fcm_token=form.cleaned_data["password2"]
            ...
            if result['success']:
                try:
                    ...
                    user.intity.fcm_token=fcm_token
                    user.intity.name=name
                    user.save()
